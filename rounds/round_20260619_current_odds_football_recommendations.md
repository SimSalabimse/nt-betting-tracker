...
**Processed**: 2026-06-19 17:50 CEST (updated 17:52 with Exploration section + 3rd bet)  
...
| 3 | Ranheim vs Lyn | Scorer begge lag i 1. omgang Ja | 3.15 | 8 | +8-15% | **Exploratory on new odds type** (half-time BTTS). Expected open, high-tempo game. True prob both score in 1H ~39-44% >> 31.7% implied. Small stake to test new market per playbook exploration rule. Uncorrelated to main Over 2.5. |

**Total New Stake / Risk**: 30 NOK  
**New Pending Total**: 30 NOK (~7.6% of 396.10 equity) — well within strict discipline (<10-12% total pending).  
**Blended Portfolio EV**: Positive ~9-15% across legs. Low correlation (win + goals + half BTTS).  
**Risk Management**: Small flat stakes, strict EV filter post deep research, no parlays. Includes 1 small exploratory on new odds type to follow "Exploration & Balance" rule.
...
- **New football props**: Several flagged. The half-time BTTS Ja @3.15 in Ranheim-Lyn cleared filters (open game expected) and is added as small exploratory #3. Other props (full time BTTS, time of goal, player anytime) were close but kept out to control total pending risk.
- **Kuwait match**: Skipped — insufficient reliable recent form/H2H/xG data for deep research.

This directly addresses the exploration rule while keeping risk disciplined. Future rounds will continue allocating small exploratory stakes to athletics H2H and exotic props when they pass Stage 2.

## Workflow Compliance & Next Steps (Executed by nt-betting-workflow)

1. This recommendations file **updated** with Exploration section + 3rd bet and pushed.
2. bet_log.csv: 1 additional Pending row appended (now 3 total new; full fetch + SHA verified, append-only).
3. current_bankroll.md: Updated with new pending risk 30 NOK, liquid recalculated, note added.
...
3. **Ranheim vs Lyn** — Both teams to score in 1st half (Ja) @ **3.15** for **8 NOK** (exploratory new odds type)
...
This maintains full compliance with nt-betting-workflow, playbook 2026-06-19 update, Exploration & Balance rule, and successful push workflow.