...
def coverage_critical_lines(
    ...
    out: list[dict[str, Any]] = []
    for line in queue:
        if is_coverage_critical(line):
            out.append(dict(line))
    # Prefer higher promo when ordering later
    out.sort(
        key=lambda r: float(r.get("promo_score") or r.get("promotion_score") or 0.0),
        reverse=True,
    )
    return out
...
def merge_candidates(
    ...
    *,
    ...
    # Family cap ≤2 (normative)
    family_counts_sel: Counter[str] = Counter()
    after_family: list[dict[str, Any]] = []
    for c in kept:
        fam = str(c.get("market_family") or "other")
        if family_counts_sel[fam] >= max_family:
            dropped.append({**_public(c), "drop_reason": "family_cap"})
            continue
        family_counts_sel[fam] += 1
        after_family.append(c)

    # Soft open_full drops (prefer drop when shortlist still large enough)
    ...
    for c in after_family:
        if c.get("_soft_open_family"):
            ...
            after_open.append(c)
    # Add open-full back only if needed to approach shortlist_min
    for reason, c in pending_open:
        if len(after_open) < shortlist_min:
            cc = dict(c)
    ...
    if multi_sport_board and soft_sport_cap > 0:
        ...
        for c in deferred_sport:
            # Enforce only while shortlist would stay ≥ shortlist_min after drops
    ...
    cov = coverage_critical_lines(queue)
    for line in cov:
        k = evidence_pair_key(str(line.get("match") or ""), str(line.get("selection") or ""))
        ...
        primary.append(
            {
                ...
                "promo_score": fnum(line.get("promo_score") or line.get("promotion_score")),
            }
        )
...
def _public(c: Mapping[str, Any]) -> dict[str, Any]:
    agents = _agents_list(c.get("scan_agents") or [])
    return {
        "match": c.get("match"),
        ...
        "scan_agents": agents,
        "scan_agent": _render_scan_agent(agents),
