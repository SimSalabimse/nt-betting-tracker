...
def _slug(s: str) -> str:
    return re.sub("[^a-z0-9]+", "_", (s or "").lower()).strip("_")[:40]
...
def _is_correct_score_blob(blob: str, mk: str) -> bool:
    ...
    return bool(
        re.search(
            "riktig resultat|korrekt resultat|correct score|korrekt score",
            blob,
        )
    )
...
def _is_totals_blob(blob: str, mk: str) -> bool:
    ...
    # ML-looking blobs without an explicit totals token are not totals.
    if (
        "vinner" in blob
        or "to win" in blob
        or mk in ("match result", "match_result", "winner", "ml")
    ):
        if not (
            "totalt" in blob
    ...
    has_total_token = (
        ...
        or (
            ...
            and any(
                x in blob
                for x in ("mål", "goal", "game", "point", "corner", "run")
            )
        )
        or (
            re.search(r"(?<![/\w])over\b", blob) is not None
            and re.search(r"\d", blob)
        )
        or (
            re.search(r"(?<![/\w])under\b", blob) is not None
            and re.search(r"\d", blob)
        )
    )
...
def _is_handicap_blob(blob: str, mk: str) -> bool:
    if "correct score" in mk or _is_correct_score_blob(blob, mk):
        return False
    if "handicap" in mk or "handikap" in mk or mk in (
        "hc",
        "set handicap",
        "map handicap",
    ):
...
def market_family(
    ...
    if (
        "btts" in blob
        or "begge lag" in blob
        or "both teams" in blob
        or mk == "btts"
    ):
