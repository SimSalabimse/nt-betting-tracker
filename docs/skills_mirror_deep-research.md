...
description: >
  ...
  candidates, hard cap 15). Exa both-sides + optional Firecrawl → structured
  evidence packs with form-continuity / opposite-side / ranking fields via
  atomic pack writer (scripts/write_deep_research_pack.py). Use when user runs
  /deep-research, says "deep research shortlist", "Stage 2 packs", "research
  primary worklist", "Exa packs for candidates", or when /daily-run reaches
...
metadata:
  short-description: "Stage 2 primary-worklist Exa packs via atomic helper"
---
...
1. `Read` repo root **`AGENTS.md`** — ESR Stage 0–4 + form continuity + Stage 2 scope law.
...
3. Confirm input list is **primary worklist ≤15** only:
   ...
   - Explicit named lines that are on that worklist / coverage_critical / Stage 3b expansion, **or**
...
5. Force real tools: Exa, optional Firecrawl, CLI, helper script. Do not simulate packs.
6. **Live anchor recipe** (working-tree ledger only — never archives):
...
| Exa searches | **4–6** standard · 3–4 tight · 3–5 expansion |
| Firecrawl scrape | **0–2** pages (search discover + scrape HQ only); **0** agent/crawl loops |
| Stage 2 batch | **≤ 45 min** hard for primary pass (soft target ≤ 35 min if parallel) |
| Stage 3b expansion | Separate ≤ 20 min; does not steal primary 45 |
...
2. Live anchor check (status + live bets).
3. Exa both sides (favourite **and** underdog / home **and** away): form, H2H, ranking, injuries/lineups.
4. Optional Firecrawl scrape 0–2 HQ pages when snippets thin.
5. Assemble **8 sections** + honest `p_model` + full ESR payload.
6. **Atomic write only:**
...
7. Optional human MD: `outbox/deep_research/<slug>.md` — **required** for Strong/Acceptable and any `flip_risk_suspected`.
8. Optional: `python run_nt.py research critique evidence/<file>.json --odds …`
9. Append `outbox/DEEP_RESEARCH_BATCH.md` row.
...
| # | Section | Pack home |
|---|---------|-----------|
| 1 | Match context | `deep_research.match_context` |
...
Also always: `summary`, sources with **non-empty takeaways** (≥4, ~≥8 chars each), honest `p_model`.
...
1. Write/update `outbox/DEEP_RESEARCH_BATCH.md` — table: match, selection, verdict, p_model, form_continuity_triggered, opposite one-liner, tooling.
2. Recap: packs written this batch have `(match, selection)` ⊆ primary worklist keys; extras = process miss.
3. Hand back to `/daily-run` Stage 3 (`research ready` → `recommend`).
