...
## ANTI-SHORTCUT (binding)

When invoked from `/daily-run` Stage 2:
- Work **only** the primary worklist ≤15 (not full dump).
- Load **MIC first** for each seat; note `process_miss` honestly.
- Both-sides + form-continuity fields required on every pack.
- **Final pack write** only via `scripts/write_deep_research_pack.py` (payload → atomic write).
- Forbidden: ad-hoc `_write_deep_packs_YYYYMMDD.py` as the sole Stage 2 path that skips this skill.
...
4. **Optional lake reads** (Stage 2 only — **never Stage 1x MIC**): if sibling `nt-data-platform` is installed and tracker `data_platform.enabled` is true, you **may** attach supplemental history via the thin adapter. **Do not** import `nt_data` into MIC fetch code or Firecrawl paths.

```powershell
# Soft-fail if package missing or enabled:false
python -c "from nt.data_platform import is_available, get_client; from nt.config import load_config; c=get_client(load_config()); print('lake', bool(c)); print(c.team_form(team='...', sport='football', league='E0') if c else None)"
# Useful calls when client non-None (football strong; tennis player_form when available):
#   c.team_form / c.league_goal_rates / c.suggest_lambdas  (api_version 0.1)
#   c.player_form(...) for tennis when implemented
```

| Rule | Detail |
|------|--------|
| **When** | MIC thin (≤C), missing form depth, or football/tennis historical continuity useful |
| **How to cite** | Pack `sources[]` takeaway e.g. `"lake:team_form E0 last8 WDL …"` · `data_coverage.evidence_quality_notes` may say `lake:form` / `lake:league_rates` |
| **Never** | Sole edge / sole `p_model` · invent rows when client is None · treat book closes as Oddsen CLV · call lake during Stage 1x |
| **Thin lake** | Baseball / snooker / LoL / CS2 may only have **manual seed** rows — say so; do not overclaim multi-season depth |
| **Budget** | ≤ **30 s** wall-clock per seat for lake calls; skip if lake unavailable |

5. **Optional Exa / HQ web** both sides when MIC thin (grade ≤ C, missing critical fields, `process_miss: true` after failed free fetch, or non-v1 sport skeleton): favourite **and** underdog / home **and** away — form, H2H, ranking, injuries/lineups.
6. Optional Firecrawl scrape 0–2 HQ pages when snippets thin (never for inventing MIC body retroactively without card write; never claim free MIC success when card is process_miss F).
7. Assemble **8 sections** + honest `p_model` + full ESR payload + **`data_coverage` / `evidence_quality`**.
8. **Atomic write only:**
...
9. Optional human MD: `outbox/deep_research/<slug>.md` — **required** for Strong/Acceptable and any `flip_risk_suspected`.
10. Optional: `python run_nt.py research critique evidence/<file>.json --odds …`
11. Append `outbox/DEEP_RESEARCH_BATCH.md` row.
...
- **Lake optional** at Stage 2 only via tracker `nt.data_platform` adapter — **never** on Stage 1x MIC / Firecrawl fetch; never sole edge; book closes ≠ Oddsen CLV.
