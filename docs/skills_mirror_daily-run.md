...
## ANTI-SHORTCUT LAW (binding — process miss if violated)

**Full `/daily-run` is multi-agent by design.** Speed, thin MIC, or “I already know the board” does **not** authorize collapsing agents into the main thread.

| Stage | REQUIRED | FORBIDDEN |
|-------|----------|-----------|
| **1b Scan A/B/C(+D)** | `spawn_subagent` (or host equivalent) **in parallel** for A, B, C, and D when armed — each writes its own `outbox/scan_agent_{a,b,c,d}_YYYY-MM-DD.jsonl` | Main inventing all scan JSONL alone while claiming multi-agent ran |
| **2 Deep** | Invoke **`/deep-research` skill** on primary worklist only; packs via **`python scripts/write_deep_research_pack.py`** | Ad-hoc one-off pack scripts as the sole Stage 2 path; bare `research write-pack` as final pack write |
| **3.1 Decisions** | `spawn_subagent` **Edge ∥ Guardian ∥ Quality** — their MD + `quality_veto_*.json` | Main writing all three agent files alone as theater |
| **3.1z** | CLI `apply-quality-veto` before recommend | Hand-nulling `p_model` |

**Only allowed sequential fallback:** if `spawn_subagent` is unavailable in the host, run A→B→C(→D) and Edge→Guardian→Quality **as distinct role-bound steps** with **separate artifacts**, and log `agent_mode: sequential_fallback` in `MULTI_AGENT_SHORTLIST.md` / `TRI_DECISION_*.md`. Still **not** a single pass inventing everything.

**Self-check before PLACE_THESE:** If scan/decision artifacts were not produced by separate agents (or explicit sequential_fallback), the run is a **process miss** — say so in the final reply; do not present it as a full multi-agent day.
...
### Lake ban on Stage 1x (hard)

**Do not** import `nt_data`, call `nt.data_platform.get_client`, or query the historical lake during Stage **1x** MIC build/top-up. MIC uses free live sources (Firecrawl-first) only. Lake form/rates belong in **Stage 2** `/deep-research` (optional) or offline night jobs (`nt-data collect/normalize/features`) — never on the MIC 8‑minute budget.

**Multi-sport free pipeline (Firecrawl-first):** live parsers + URL discovery for all `v1_sports` — football, tennis, esports, snooker, darts, baseball. Desk config: `research.match_intel.allow_network: true` + `fetch.prefer: firecrawl`. Discovery uses **Firecrawl web search** first (not fragile Flashscore `/search/` SPA scrape). CI/unit tests pass `--no-network`.

```powershell
# Desk (network ON by default in config.yaml):
python run_nt.py research match-intel --odds <odds_file> --allow-network --fetch firecrawl
# single match / force rebuild:
python run_nt.py research match-intel --match "Team A vs Team B" --sport football --allow-network --force
# offline/CI only:
python run_nt.py research match-intel --odds <odds_file> --no-network
```
...
**Parallelism (mandatory when spawn available):** **A ∥ B ∥ C (∥ D)** via `spawn_subagent` — not main-thread fan-fiction. Sequential fallback only if spawn unavailable: **A → B → C → (D if armed)** with `agent_mode: sequential_fallback`. Wait budget: **≤12 minutes** for the **entire** scan layer including D.
...
**Speed law:** ≤ **10 minutes** wall-clock. **Must spawn** Edge **∥** Guardian **∥** Quality (same anti-shortcut law as scan). Sequential fallback only if spawn unavailable.  
...
- **ANTI-SHORTCUT LAW** — spawn scan A/B/C(+D) and decision Edge/Guardian/Quality; run `/deep-research` skill; atomic `write_deep_research_pack.py` only for pack finals.
- Live recommend by default; dry-run only on request.
- **0b can-bet halt** after refresh — stop research when `can_bet: false`.
- **1x MIC** before deep — Firecrawl-first live desk (`--allow-network --fetch firecrawl`); hard top-up after 1c; budget ≤8 min.
- **Stage 1b** A∥B∥C always (spawned); D only when any match ≥41 Candidate lines.
