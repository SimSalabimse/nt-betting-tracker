...
description: >
  ...
  settle → odds dump → market-scan → board+light → promising deep queue →
  Exa both-sides deep packs → recommend best +EV (soft dogs not guilty by
  default; short 1.40–1.80 OK) → expand if large board & <2 picks →
...
metadata:
  short-description: "Full day ESR Stage 0–4 — find best edges, expand before empty"
---
...
> **Repo mirror:** keep `~/.grok/skills/daily-run/SKILL.md` in sync with this file. Desk pointer: `docs/DESK_SKILLS.md`. Philosophy: `docs/RESEARCH_RESET_SIMPLE_EFFECTIVE_2026-07-25.md`.
...
4. Evidence packs: `evidence/*.json`
5. Coverage Health: `data/state/coverage_health.json`
6. Slip: **`outbox/PLACE_THESE.md`**
7. Status: `data/state/status.md` · `risk.json`
8. Reasoning: why · support · main risk (+ near-misses)
9. Expansion done? (if large board &lt;2 initially)
10. place-ack / abandon ids if place session ran
