...
**As of:** 2026-07-27 (data-first MIC / soft-reset / three Decision Agents stack + prior adaptive scan + closed-loop ControlSignals / PhaseState)  
...
| R-S2-14 | **MIC scrape fragility** — Firecrawl/NT/Flashscore/FotMob HTML drift, alias misses, rate limits → grade **F** skeletons or stale form/H2H | **Partially mitigated (c7e81b09 PR-0…5 + PR-6 skills):** multi-sport live parsers (football/tennis/esports/snooker/darts/baseball) + fetch layer + URL discovery + `process_miss` taxonomy (fetch fail vs `thin_public`); offline fixtures; skill guidance ≤15 live matches, Firecrawl/Playwright install notes, Quality notes must cite `process_miss_reason` on hard_veto; soft pressure only while `require_for_deep: false` | **Residual:** live scrape can still yield thin/wrong MIC on a given day; HTML/XHR drift re-breaks parsers; agents that invent form/H2H when card is F/`process_miss`/missing recreate the pre-MIC failure mode; no permanent free-source SLA; premature `require_for_deep` flip would hard-block v1 seats on bad scrape days |
| R-S2-15 | **Quality-veto misuse** — Challenger emits loose `hard_veto`s, Main invents vetoes not in JSON, or `apply-quality-veto` skipped so Challenger power is zero | Closed-enum reasons; CLI-only pack mutation (null `p_model`); KD-place-law (Main never hand-edits packs); skill requires 3.1z + `quality_veto_applied_*.json`; skill_smoke CLI help + TRI phrases | **Residual:** operator can still skip 3.1z or paste invented veto JSON; undoing a bad apply needs discipline; soft_demote remains advisory (notes-only) |
| R-S2-16 | **Soft-reset pending abort** — operator runs `--confirm` with open Pending/ConfirmedPlaced, or uses `--allow-pending` unsafely; equity/era reset while tickets still open | Script **aborts by default** when open risk &gt; 0; operator path: settle/abandon → refresh → `--dry-run` → `--confirm`; skill_smoke exercises `--help` + `--dry-run` | **Residual:** `--allow-pending` exists for tests; force-confirm with open risk corrupts the clean `esr_data_v1` era narrative; abandoned tickets need honest settle later |
| R-S2-17 | **Exa transition incomplete (E1–E5)** — `exa_mode` still optional; `require_for_deep` still false; football MIC not hard-required for deep | Documented exit criteria E1–E5 in design + `docs/EXA_RESEARCH_USAGE.md`; MIC still built (soft); PR6 deferred until gates green (**do not flip config early**) | **Residual:** packs can still lean on Exa narrative without strong MIC; premature PR6 flip would hard-block v1_sports deep on scrape bad days |
