...
| R-S2-14 | **MIC scrape fragility** — Firecrawl/NT/Flashscore/FotMob HTML drift, alias misses, rate limits → grade **F** skeletons or stale form/H2H | Offline fixtures + multi-source pipeline + grade F fail-soft; scan agents tag `mic:missing` / cite only present cards; soft pressure only until PR6 `require_for_deep` | **Residual:** live scrape can still yield thin/wrong MIC on a given day; agents that invent form/H2H when card is F/missing recreate the pre-MIC failure mode; no permanent free-source SLA |
