...
| **Darts** | 7-8% | 1.70 - 2.80 | Match winner (form, H2H, averages, streaks); Legs handicap | Veteran vs inconsistent; avoid fatigue spots in long events | Selective volume encouraged | Low | Highly profitable when selected (per history) | **HIGH** - Prioritize variety; test when +EV; conclude when data sufficient (see 2026-06-16 playbook update) | Excellent diversifier; consistent player metrics |
| **Snooker** | 8% | 1.70 - 3.20 | Match winner (form, ranking diff, H2H in format, motivation); Frame handicap / total frames | Long matches: mental/tactical edges; motivated underdogs | Selective when clear +EV | Very Low | Positive signals in history | **HIGH** - Prioritize variety; test when +EV; conclude when data sufficient (see 2026-06-16 playbook update) | Strong predictive factors; good for learning |
...
- **Mandatory Exploration**: In any round with 3+ solid +EV opportunities in core sports (football/tennis), allocate at least **1 exploration slot** to Darts, Snooker, or other low-volume positive-ROI sports if any +EV (even at slightly lower bar ~5-6% rough EV) is available. Goal: Test to learn/confirm profitability. "If not tested, how can we learn?" **Updated 2026-06-16**: This is now dynamic — prioritize **variety across different sports and bet types**. Use HIGH priority flags as soft signals. Conclude focused exploration on a sport/bet type when sufficient data (volume + clear patterns from deep dives/ROI) has been gathered. Avoid perpetual forcing of any single sport (Snooker or otherwise).
...
- **2026-06-16**: Softened HIGH priority language and Exploration Rules per user feedback (see additive section in playbook.md). Changed from "Force inclusion" / strict quota to dynamic variety-focused exploration with data-driven conclusions when enough data gathered. Updated table cells for Darts/Snooker. bet_log.csv quoting fix also applied globally.

**This file is now the go-to for edge parameters. playbook.md will link here and focus on process.**

## 2026-06-16 Additive Improvement Note
**Dynamic Exploration Update (playbook-aligned)**: HIGH Exploration Priority for Darts and Snooker remains as positive historical signal and good diversifier, but is no longer interpreted as mandatory/forced inclusion every round. The system should naturally try different sports and bet types based on available +EV lines in Stage 1/2. When deep dives and ROI tracking show sufficient data volume and stable patterns for a sport or bet type (typically 10-20+ bets), conclude that exploration phase and shift focus. This prevents getting "stuck" on any example sport (Snooker was illustrative) and encourages broad, conclusion-oriented learning. Future table updates and round recommendations will follow this flexible approach. See full details in the 2026-06-16 section of playbook.md.
